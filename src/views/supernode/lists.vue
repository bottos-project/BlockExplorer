<template>
  <!-- <div class="main"> -->
            ...
            <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" v-for="item in list" key="item.id">
              <div class="item">
                ...
                <div class="box">
                  ...
                  <div class="fr">
                    <div class="ticket">
                      {{item.votes / Math.pow(10,8)}} {{$t('supernode.lists_ticket')}}<br />
    ...
    <table width="100%" align="center">
      <!-- <caption><h3>超级节点排名</h3></caption> -->
      <thead> 
        <tr>
          <th>{{$t('supernodeDetail.ranking')}}</th>
          <th>{{$t('supernodeDetail.account')}}</th>
          <th>{{$t('supernodeDetail.cast_vote')}}</th>
          <th>{{$t('supernodeDetail.quality_deposit')}}</th>
          <th>{{$t('supernodeDetail.table10')}}</th>
        </tr>
      </thead>
      <tbody> 
        <tr v-for="(item,index) in tableData">
          <td>{{index + 1}}</td>
          <td><router-link :to="'/supernode/detail/'+item.delegate" class="more">{{item.delegate}}</router-link></td>
          <td>{{item.votes}}</td>
          <td>{{item.staked_balance}}</td>
          <td>{{item.un_claimed_reward}}</td>
        </tr>
      </tbody>
    </table>
      ...
      methods:{
        ...
        fetchData() {
          ...
          queryNodeList(para).then(response => {
            ...
            response.map((item)=>{
              let votes = item.votes / Math.pow(10,8)
              let staked_balance = item.staked_balance / Math.pow(10,8)
              let un_claimed_reward = item.un_claimed_reward / Math.pow(10,8)
              item.votes = votes || 0
              item.staked_balance = staked_balance || 0
              item.un_claimed_reward = un_claimed_reward || 0
            })
            this.tableData = response.sort((x,y)=>{
              let xvotes = x.votes / Math.pow(10,8)
              let yvotes = y.votes / Math.pow(10,8)
              if (xvotes < yvotes) {
                return 1;
              }
              if (xvotes > yvotes) {
