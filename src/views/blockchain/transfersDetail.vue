<template>
  <div class="main">
    <div class="border">
        <div class="title"><h3><span>{{$t('transfersDetail.title')}}</span></h3></div>
        <div class="detail">
            <ul>
                <li>
                    <div class="tit">{{$t('transfersDetail.state')}}</div>
                    <div class="con">
                ...
                <li class="row">
                    <div class="tit">{{$t('transfersDetail.transactionsID')}}</div>
                    <div class="con">{{Detail.transactionId}}</div>
                </li>
                <li>
                    <div class="tit">{{$t('transfersDetail.block')}}</div>
                    <div class="con">
                ...
                <li class="row">
                    <div class="tit">{{$t('transfersDetail.time')}}</div>
                    <div class="con">
    ...
    <div class="border">
        <div class="title"><h3><span>{{$t('transfersDetail.title2')}}</span></h3></div>
        <div class="detail">
            <ul>
                <li>
                    <div class="tit">{{$t('transfersDetail.name')}}</div>
                    <div class="con">
                        {{Detail.contract}}
                    </div>
                </li>
                 <li class="row">
                    <div class="tit">{{$t('transfersDetail.method_name')}}</div>
                    <div class="con">
                        {{Detail.method}}
                    </div>
                </li>
...
<script>
import {queryTradeDetail} from '@/api/blockchain_transfers'
      ...
      methods:{
        fetchData() {
          ...
          queryTradeDetail(para).then(response => {
            ...
            if (param && "memo" in param){
                console.log(param.memo)
                let memoValue = param.memo
                delete param.memo
                param["memo"] = memoValue
            }
            if (param && "amount" in param){
                param["amount"] = param.amount / Math.pow(10,8) + "BTO"
            }
            if(response.data.param.memo){
              let charLength = 0
              for(var i in response.data.param.memo){
                charLength++
              }
              if(charLength > 32){
                delete response.data.param['memo']
              }
            }
